...
#PBS -N camelina_RNAseq_joelle_samtools_strandparse
#PBS -e camelina_RNAseq_joelle_samtools_strandparse.error
#PBS -o camelina_RNAseq_joelle_samtools_strandparse.out
#PBS -M dorn@umn.edu
#PBS -m abe
...
###joelle_RNAseq
#These reads are all of the joelle trimmed reads from this experiment concatenated together to try and find sense/antisense start/stop points in the FLC loci 
# Sense (+) strand 
...
samtools view -b -f 163 joelle_RNAseq_sorted.bam > fwd1.bam
samtools index fwd1.bam

#2
samtools view -b -f 83 joelle_RNAseq_sorted.bam > fwd2.bam
samtools index fwd2.bam


# Combine alignments that originate on the forward strand.
samtools merge -f joelle_fwd.bam fwd1.bam fwd2.bam
samtools index joelle_fwd.bam
...
samtools view -b -f 99 joelle_RNAseq_sorted.bam > rev1.bam
samtools index rev1.bam

samtools view -b -f 147 -F 16 joelle_RNAseq_sorted.bam > rev2.bam
samtools index rev2.bam

#
# Combine alignments that originate on the reverse strand.
#
samtools merge -f joelle_rev.bam rev1.bam rev2.bam
samtools index joelle_rev.bam